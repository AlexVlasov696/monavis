...
if ( ! class_exists( 'Taxonomy_Term_Image' ) ) :
    ...
    class Taxonomy_Term_Image {
        ...
        static function instance() {
            static $object = null;
            $taxonomy = 'category';
            $js_dir_url = get_bloginfo('template_directory') . '/assets/js/';
            if ( is_null( $object ) ) {
                $object = new Taxonomy_Term_Image($taxonomy, $js_dir_url);
            }
            return $object;
        }
         ...
         */
        public function __construct($tax, $js_dir) {
            // default labels
            $this->taxonomy = $tax;
            $this->js_dir_url = $js_dir;

            global $themename;

            $this->labels = array(
                'fieldTitle'       => __( 'Image', $themename),
                'fieldDescription' => __( 'Upload image', $themename),
                'imageButton'      => __( 'Select Image', $themename),
                'removeButton'     => __( 'Remove', $themename),
                'modalTitle'       => __( 'Select Image', $themename),
                'modalButton'      => __( 'Select', $themename),
            );
